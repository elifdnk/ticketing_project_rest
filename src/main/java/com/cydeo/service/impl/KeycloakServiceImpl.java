...
public class KeycloakServiceImpl implements KeycloakService {
    ...
    private final KeycloakProperties keycloakProperties; // we injected this class because we use these variables.(ex: look at line 52)
    ...
    @Override
    public Response userCreate(UserDTO userDTO) { //overriding keycloak service. whatever doing in keycloak we doing with spring boot now

        CredentialRepresentation credential = new CredentialRepresentation();
        credential.setType(CredentialRepresentation.PASSWORD);
        credential.setTemporary(false);
        credential.setValue(userDTO.getPassWord()); //capture password and assigning to value to keycloak
        ...
        RoleRepresentation userClientRole = realmResource.clients().get(appClient.getId())
                .roles().get(userDTO.getRole().getDescription()).toRepresentation(); //it goes to keycloak and check all roles it is taking getrole, whenever it matches role in the keycloak it is assign that role keycloak under that user.
